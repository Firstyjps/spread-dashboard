...
class AsterCollector(ExchangeAdapter):
    name = "aster"

    def __init__(self, base_url: str | None = None, symbols: list[str] | None = None):
        self.base_url = (base_url or settings.aster_futures_base_url).rstrip("/")
        self._symbols = symbols or ["XAUUSDT", "PAXGUSDT"]
    ...
    async def fetch_ticker(self, symbol: str) -> NormalizedTick | None:
        ...
        try:
            ...
            async with session.get(url, params=params) as resp:
                if resp.status != 200:
                    log.warning("aster_ticker_http_error", symbol=symbol, status=resp.status)
                    return None
            ...
            log.warning("aster_ticker_error", symbol=symbol, error=str(exc))
            return None
